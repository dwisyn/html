...
<tr><td>{{ $m->iduser }}</td>
	...
	<td>{{ $m->password }}</td>
	<td>{{ $m->jarak }}</td>
	<td>{{ $m->rumah->idrumah }}</td>
	<td>{{ $m->rumah->alamat }}</td>
	<td>{{ $m->rumah->uuid }}</td>
	<td>{{ $m->rumah->mac }}</td>
	<td>{{ $m->email }}</td>
	<td>{{ $m->pro }}</td>
...
<td align="center" width="30px">
{!! Form::open(array('url' => env('APP_URL').'akun_pelanggan/'.$m->iduser ,'method'=>'delete','style' => 'display:inline')) !!}
